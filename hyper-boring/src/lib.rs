    ...
    T: AsyncRead + AsyncWrite + Unpin,
{
    fn poll_read(
        ...
        match &mut *self {
            MaybeHttpsStream::Http(s) => hyper::rt::Read::poll_read(Pin::new(&mut TokioIo::new(s)), ctx, buf),
            MaybeHttpsStream::Https(s) => hyper::rt::Read::poll_read(Pin::new(&mut TokioIo::new(s)), ctx, buf),
        }
    }
}
    ...
    T: AsyncRead + AsyncWrite + Unpin,
{
    fn poll_write(
        ...
        match &mut *self {
            ...
            MaybeHttpsStream::Https(s) => hyper::rt::Write::poll_write(Pin::new(&mut TokioIo::new(s)), ctx, buf),
        }
    }
    ...
    fn poll_flush(mut self: Pin<&mut Self>, ctx: &mut Context<'_>) -> Poll<io::Result<()>> {
        match &mut *self {
            ...
            MaybeHttpsStream::Https(s) => hyper::rt::Write::poll_flush(Pin::new(&mut TokioIo::new(s)), ctx),
        }
    }
    ...
    fn poll_shutdown(mut self: Pin<&mut Self>, ctx: &mut Context<'_>) -> Poll<io::Result<()>> {
        match &mut *self {
            ...
            MaybeHttpsStream::Https(s) => hyper::rt::Write::poll_shutdown(Pin::new(&mut TokioIo::new(s)), ctx),
        }
    }
